(() => {
  ...
  el("launch").addEventListener("click", () => {
    ...
    if (window.novetMobileMode) build.config.devicePixelRatio = Math.min(1.25, 1280 / Math.max(innerWidth, innerHeight));
    ...
    script.onload = () => {
      createUnityInstance(canvas, build.config, value => {
        ...
        instance = game;
        window.NovetRaceRecording.attach(game);
        if (failed) return;
        if (window.novetMobileMode) window.NovetMobileControls(game);
        else window.NovetGamepadBridge(game, text => { el("controller-status").textContent = text; });
  ...
  function updateController() {
    if (instance || window.novetMobileMode) return;
    const pads = navigator.getGamepads ? Array.from(navigator.getGamepads()) : [];
    const next = pads.some(pad => pad && pad.connected && !/LED Controller|RGB Controller|Lighting Controller/i.test(pad.id));
    if (next !== connected) {
      connected = next;
